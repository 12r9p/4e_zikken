...
=== (5) サイリスタ単相半波整流回路
制御角（点弧角） $alpha$ で導通を開始し、電源電圧が負になる $pi$ で電流が遮断される。周期は $2 pi$ である。
- *平均値 $V_d$*：
  $
...
周期は $pi$ となり、制御角 $alpha$ から $pi$ までの区間で導通する。
- *平均値 $V_d$*：
  $
...
=== (1) 3-1 単相半波整流回路
- *特性変化*：
  平滑コンデンサを接続しない場合（$V_"in" = 40, 50, 60 "V"$），波形率は $1.57 ~ 1.59$，波高率は $1.98 ~ 2.02$ となっている。これは理論値である波形率 $F_f = pi / 2 approx 1.57$，波高率 $F_c = 2 approx 2.00$ と非常によく一致している。
...
=== (2) 3-2 単相ブリッジ整流回路
- *特性変化*：
  コンデンサなし時、実測の波形率は $1.12$、波高率は $1.41 ~ 1.45$ であり、理論値 $F_f = pi / (2 sqrt(2)) approx 1.11$、$F_c = sqrt(2) approx 1.41$ に極めて近い。コンデンサ接続時には波形率 $1.00$、波高率 $1.08$ となり、単相半波のときよりも脈動がさらに小さい高品質な直流となっている。
- *誤差の分析*：
  実測値は理論値に比べ $-3\%$ から $-7\%$ 程度の負の誤差を有する。ブリッジ整流回路では常に2個のダイオードが直列に導通するため、ダイオードの順方向電圧降下が2倍（約 $1.4" V"$）になり、単相半波よりも電圧降下誤差の影響がわずかに大きくなっている。

=== (3) 3-3 三相半波整流回路
- *特性変化*：
  コンデンサなし時の実測波形率は $1.16 ~ 1.17$、波高率は $1.48 ~ 1.54$ である。
  コンデンサあり時の実測波形率は $1.01$、波高率は $1.05 ~ 1.08$ となり、平滑作用が顕著に機能している。
- *誤差の分析*：
...
=== (4) 3-4 三相ブリッジ整流回路
- *特性変化*：
  コンデンサなし時の実測波形率は $1.00 ~ 1.01$、波高率は $1.08 ~ 1.09$ と、コンデンサ未挿入の状態であっても極めて直流に近い優れた波形特性を示す。
  コンデンサあり時には波形率 $1.02$、波高率 $1.03 ~ 1.04$ となり、ほぼ完全な平坦直流が得られている。
- *誤差の分析*：
...
=== (5) 3-5 サイリスタ半波整流回路
- *特性変化*：
  $alpha = 45"°" arrow 90"°" arrow 120"°"$ に増加させるに伴い、実測の波形率は $1.75 arrow 2.22 arrow 2.81$、波高率は $2.23 arrow 2.83 arrow 3.36$ と急激に増大している。これは、点弧角を広げることで導通角が狭まり、出力電圧が不連続なパルス状波形となるためである。
- *誤差の分析*：
  $alpha = 45"°", 90"°"$ においては、実測値と理論値の差は $+- 6\%$ 以内と良好である。しかし $alpha = 120"°"$ では、平均値誤差率が $+15.5\%$、実効値誤差率が $+16.7\%$ と極端に大きな正の誤差が生じている。これは、導通期間が非常に狭く電流値が極めて小さくなった結果、オシロスコープのトリガータイミングのずれ、および測定ノイズの相対的増加が主な誤差原因である。

=== (6) 3-6 混合ブリッジ整流回路
- *特性変化*：
  $alpha = 45"°" arrow 90"°" arrow 120"°"$ に広げると、実測波形率は $1.22 arrow 1.58 arrow 1.97$ と上昇し、出力の平滑性が損なわれていく挙動が確認できる。
- *誤差の分析*：
...
- *コンデンサありの場合*：
  ...
  これは電源電圧のピーク付近（導通角が非常に狭い期間）に限られる。この短い期間に、ダイオードを介してコンデンサへ一気に電荷を送り込むため、入力電流はピーク近傍で急峻に立ち上がり、ピークを過ぎるとダイオードがすぐ逆バイアスされるので急激に立ち下がる、鋭いパルス状（スパイク状）の波形になる。電圧のピークを過ぎると $v(t) < v_c(t)$ となり、電流は完全に 0 となる。
  この現象は、測定波形 @fig:3-1_1ph_half_cap_dc において、出力電圧のピーク手前で急峻なパルス状電流（アノード電流）が観測されている動作と完全に一致する。

== 6. 三相整流回路の入力電流が食パン波形になる理由
// 三相整流回路の入力電流が食パンのような波形になる理由を説明してください。
三相ブリッジ整流回路（3-4）の動作時において、交流入力側の相電流波形が純粋な正弦波から著しく逸脱し、正・負それぞれに頂部が平坦で角ばった「食パンのような波形」（あるいはブロック状の断続波形）になる理由は、主に以下の二つの要因によるものである。

1. *ダイオードのスイッチング動作と120度導通特性*：
  三相ブリッジ回路では、回路図上で最も電位の高い相につながる上側ダイオード1個と、最も電位の低い相につながる下側ダイオード1個が同時にオンになり、その2本を通って電流が交流側から直流側へ流れる。したがって、電流の経路は「高い相の上側ダイオード -> 負荷 -> 低い相の下側ダイオード」という形で順次切り替わる。
  各相電流は、対応する上側または下側ダイオードがオンのあいだだけ流れ、1周期の中で120°ずつ導通する。測定した線電流では、ピークが隣り合って2つ現れるが、これは60°ごとに導通するダイオードの組が切り替わるためであり、どのダイオードがオンになるかが変わるたびに電流の向きと流れる相が入れ替わる。
  この断続的なスイッチング動作により、入力電流はなだらかな正弦波にはならず、急峻に立ち上がり平坦な頂部を持つ矩形波になる。
