...
1. *ダイオードのスイッチング動作と120度導通特性*：
  三相ブリッジ回路では、6個のダイオードが最も線間電圧の高い相同士を導通させるよう順次切り替わる。各相から流れ込む入力電流は、上側ダイオードがオンのときに正方向に, 下側ダイオードがオンのときに負方向に流れる。それぞれ1周期のうち、電気角で120°ずつの期間しか導通せず、残りの期間は電流が0となる。この断続的なスイッチング動作により、入力電流はなだらかな正弦波にはならず、急峻に立ち上がり平坦な頂部を持つ矩形波になる。

2. *負荷側の平滑作用による電流の定電流化と転流による波形歪み*：
  負荷のインダクタンス成分の働きにより、整流後の直流電流はほぼ一定（平滑な直流）に維持されようとする。ダイオードが導通している120°の間、このほぼ一定の直流電流がそのまま交流入力側に現れるため、電流波形の頂部がクランプされて平らになる。さらに、電源変圧器などの漏れリアクタンスによる「転流重なり現象（オーバーラップ）」の影響で、電流の立ち上がり・立ち下がりが瞬時ではなく一定の傾きを持つため、電流波形の肩がやや丸みを帯びた、まさに「食パンの山」のような形状になる。
