...
三相ブリッジ整流回路（3-4）の動作時において、電源の入力線間電圧波形が純粋な正弦波から逸脱し、頂部が平坦で角ばった「食パンのような波形」になる理由は、以下の二つの相互作用によるものである。

1. *変圧器の漏れリアクタンスとダイオードの転流動作*：
  三相ブリッジ回路では、導通するダイオードが次の相へと順次切り替わる「転流」が発生する。電源の変圧器には必ず「漏れリアクタンス」が存在するため、電流は一瞬では変化できず、転流する前後の二つの相のダイオードが同時に導通する「重なり期間（オーバーラップ期間）」が生じる。この重なり期間中、二つの相が短絡されたような状態になり、変圧器の漏れインピーダンスによる極めて深い電圧降下（転流ノッチ、転流たるみ）が生じて、電圧波形が押し潰される。
