...
以下では、実験で取得したFFTの振幅値を周波数軸へ変換して振幅スペクトルを作成した。各データの周波数軸は $f_k = k f_s/N$ とし、グラフの縦軸は振幅スペクトル、横軸は周波数とした。
...
VoiceChangerのMaincoreおよびSubcore1を用い、マイクから入力した音声をFFTし、逆FFTによって時間波形へ戻してスピーカーから出力した。サンプル数を半分にして処理した場合、出力音声は音量は小さく、一つの音として認識できない「ボコボコ」した音になった。
...
さらに、本実験では、入力信号を一度フーリエ変換・逆フーリエ変換した後にスピーカーで再生し、その音をマイクで再び取得している。このため、FFT・逆FFTの処理に加え、スピーカーとマイクの周波数特性、周囲雑音、再録音時の音量変化の影響を受ける。その結果、入力した正弦波のみでは説明できない周辺成分が現れ、振幅スペクトルの波形が乱れたと考えられる。
...
図10〜図14を比較すると、母音ごとにピークの位置や高周波側の成分分布が異なった。例えば、「あ」では、本人は600〜660 Hz付近、班員Aは100〜240 Hz付近、班員Bは500〜560 Hz付近に大きな成分が現れた。「う」では三者とも100〜330 Hz付近に大きな成分が見られたが、最大となる周波数は一致しなかった。「え」と「お」では、本人と班員Bに420〜470 Hz付近の大きな成分が見られた一方、班員Aでは100〜240 Hz付近の成分が支配的であった。また、「い」では、班員Bに約2.9 kHzの成分が現れ、他の二者とは異なる分布となった。

このような差は、母音によって声道の形状が変化し、共鳴周波数が変わることに加え、話者ごとに声帯の振動や声道の形状が異なるために生じる。したがって、母音は時間波形だけでなく、周波数領域のスペクトルの違いからも特徴付けられる。
