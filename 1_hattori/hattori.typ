...
VoiceChangerのMaincoreおよびSubcore1を用い、マイクから入力した音声をFFTし、逆FFTによって時間波形へ戻してスピーカーから出力した。サンプル数を半分にして処理した場合、出力音声は「ボコボコ」した音になった。
...
フーリエ変換による周波数解析2で取得した音声から、「あ」「い」「う」「え」「お」の各母音を取り出した。各母音の振幅スペクトルを図10〜図14に示す。
...
図10〜図14を比較すると、母音ごとにピークの位置や高周波側の成分分布が異なった。これは、母音によって声道の形状が変化し、共鳴周波数であるフォルマントが変わるためである。したがって、母音は時間波形だけでなく、周波数領域のスペクトルの違いからも特徴付けられる。

/* 検討5: 現在は本人の音声について整理している。班員の同じ母音のスペクトルを追加し、共通するピークと個人差を比較して追記する。 */
