...
#let image-paths = (
  ...
  "assets/figures/13_ハートレー型交流等価回路3.svg": "assets/ハートレー型発振回路の交流等価回路2.jpeg",
  ...
  "assets/figures/17_コルピッツ型交流等価回路3.svg": "assets/コルピッツ型発振回路の交流等価回路2.jpeg",
  "assets/figures/18_ターマン型発振回路の帰還回路.svg": "assets/ターマン型発振回路の帰還回路.jpeg",
  "assets/figures/19_ウィーンブリッジ回路.svg": "assets/JPEG画像-4FBF-9474-1D-0.jpeg",
...
ターマン型発振回路の帰還回路を図 @fig-wien-feedback に示す。$Z_1=1/(1/R_1+j omega C_1)$、$Z_2=R_2+1/(j omega C_2)$ とおくと、分圧則より帰還率は
...
を得る。特に $R_1=R_2=R$、$C_1=C_2=C$ のとき、$f_0=1/(2 pi R C)$ であり、帰還率は $beta=1/3$ となる。
...
== (1) ハートレー型発振回路
ハートレー型発振回路の実験回路を図 @fig-hartley-experiment に示す。発振周波数の理論式は $f_0=1/(2 pi sqrt(L C_2))$ である。測定結果および誤差率を表 1 に示す。
...
表 1 より、$C_2$ が小さくなるほど理論値との誤差率が大きくなり、$C_2=48.2 "pF"$ では 27.7 % に達した。コンデンサを開放したときにも 1.116 MHz で発振したことから、コイルの自己共振容量およびトランジスタの内部容量などによる寄生容量 $C_("open")$ が存在すると考えられる。
...
と求められる。そこで、各設定容量に $C_("open")$ を加えて理論周波数を補正した。補正後の結果を表 2 に示す。
...
表 2 では誤差率が 1.0 % から 3.2 % に低下した。このことから、表 1 で小容量側の誤差が大きくなった主因は寄生容量であったと判断できる。最小容量側でなお2桁の誤差が残ったのは、寄生容量を一定値と近似したこと、コイルの損失および測定器の負荷の影響によると考えられる。

== (2) コルピッツ型発振回路
コルピッツ型発振回路の実験回路を図 @fig-colpitts-experiment に示す。発振周波数の理論式は

$ f_0 = 1 / (2 pi sqrt(L (C_1 C_3/(C_1+C_3)))) $

である。測定結果および誤差率を表 3 に示す。
...
表 3 より、$C_3$ が小さいほど誤差率が大きく、48.2 pF では 24.7 %、963 pF では 0.5 % であった。小容量側では、コイルやトランジスタに由来する静電容量の比率が大きくなるため、設定値だけで計算した理論周波数との差が増加したと考えられる。開放時には発振しなかったため、ハートレー型のように開放時の発振周波数から補正容量を一意に求めることはできない。
...
可変抵抗 $R_4=20.36 "kΩ"$ に設定し、増幅率を上昇させて発振を開始したときの周波数を表 4 に示す。
...
図 @fig-bode より、位相差は 3.3 kHz で 0° となり、帰還率はこの近傍で最大となった。設計値 $f_0=3.344 "kHz"$ に対して、位相差が 0° となった実測周波数は 3.300 kHz であった。したがって誤差率は 1.3 % である。なお、離散的な測定点の中で最大帰還率 $beta=0.324$ が得られたのは 3.4 kHz であった。
近傍の測定分解能および電圧読取りのばらつきによる差とみなせる。



表 4 より、2 回の測定でともに 3.33 kHz の発振を得た。設計値 3.34 kHz に対する誤差率は 0.30 % である。測定した増幅率は $A=3.04$、帰還率は $beta=0.323$ であり、
