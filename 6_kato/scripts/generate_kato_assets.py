...
ax.plot(df_load['Load Current I_L [A]'], df_load['Terminal Voltage V_L [V]'], 
        marker='o', color='#2563EB', linewidth=2, markersize=6, label='測定値 $V_L$')
# Mark rated voltage 200V and no-load voltage 210.5V
ax.axhline(200.0, color='#94A3B8', linestyle=':', linewidth=1.5, label='定格電圧')
ax.scatter([0.0], [210.5], color='#EF4444', s=60, zorder=5, label='無負荷時')
ax.scatter([5.75], [197.5], color='#10B981', s=60, zorder=5, label='最大負荷時')

ax.set_title('同期発電機の実負荷特性', fontsize=12, pad=10)
ax.set_xlabel('負荷電流 $I_L$ [A]', fontsize=10)
ax.set_ylabel('端子電圧 $V_L$ [V]', fontsize=10)
ax.set_xlim(-0.5, 6.5)
ax.set_ylim(190, 220)
ax.grid(True)
...
ax.plot(df_inc['Field Current I_F [A]'], df_inc['Terminal Voltage V_0L [V]'], 
        marker='o', color='#0D9488', linewidth=2, markersize=5, label='界磁電流増加時（往路）')
ax.plot(df_dec['Field Current I_F [A]'], df_dec['Terminal Voltage V_0L [V]'], 
        marker='x', color='#EA580C', linewidth=2, markersize=5, label='界磁電流減少時（復路）')

ax.set_title('無負荷飽和特性曲線', fontsize=12, pad=10)
ax.set_xlabel('界磁電流 $I_F$ [A]', fontsize=10)
ax.set_ylabel('無負荷端子電圧 $V_{0L}$ [V]', fontsize=10)
ax.grid(True)
...
ax.scatter(df_sc['Field Current I_F [A]'], df_sc['Average Short Circuit Current I_s [A]'], 
           color='#4F46E5', marker='o', s=40, zorder=5, label='平均短絡電流の測定値')
...
ax.plot(fit_x, fit_y, color='#818CF8', linestyle='--', linewidth=1.5, label='一次近似直線')

ax.set_title('三相短絡特性曲線', fontsize=12, pad=10)
ax.set_xlabel('界磁電流 $I_F$ [A]', fontsize=10)
ax.set_ylabel('短絡電流 $I_s$ [A]', fontsize=10)
...
ax1.plot(df_z['Field Current I_F [A]'], df_z['Synchronous Impedance Z_s [Ohm]'], 
         marker='s', color=color_z, linewidth=2.5, markersize=5, label='同期インピーダンス $Z_s$')
ax1.set_xlabel('界磁電流 $I_F$ [A]', fontsize=10)
ax1.set_ylabel(r'同期インピーダンス $Z_s$ [$\Omega$]', color=color_z, fontsize=10)
...
ax2.plot(df_z['Field Current I_F [A]'], df_z['Open Circuit Voltage V_0 [V]'], 
         color=color_v, linestyle=':', linewidth=1.5, label='無負荷電圧 $V_0$（近似値）')
ax2.plot(df_z['Field Current I_F [A]'], df_z['Short Circuit Current I_s [A]'], 
         color=color_i, linestyle='-.', linewidth=1.5, label='短絡電流 $I_s$（近似値）')
ax2.set_ylabel('無負荷電圧 $V_0$ [V] / 短絡電流 $I_s$ [A]', color='#333333', fontsize=10)
...
ax1.set_title('同期インピーダンスの界磁電流依存性', fontsize=12, pad=10)
plt.tight_layout()
...
ax.plot(T_load, N_range, color='#6B7280', linestyle='--', linewidth=2, label='負荷特性')

# Add operating points (intersections calculated in planning)
# Weak: N=100, T=16
ax.scatter([16.0], [100.0], color='#DC2626', s=50, zorder=5)
ax.annotate('A（弱界磁）', xy=(16.0, 100.0), xytext=(18.0, 103.0),
...
ax.annotate('B（標準界磁）', xy=(12.3, 87.7), xytext=(14.3, 90.7),
            arrowprops=dict(arrowstyle="->", color='#059669', lw=0.8), fontsize=9, color='#059669')

# Strong: N=76.5, T=9.8
ax.scatter([9.8], [76.5], color='#2563EB', s=50, zorder=5)
ax.annotate('C（強界磁）', xy=(9.8, 76.5), xytext=(11.8, 79.5),
            arrowprops=dict(arrowstyle="->", color='#2563EB', lw=0.8), fontsize=9, color='#2563EB')

ax.set_title('直流分巻電動機の速度-トルク特性概念図', fontsize=11, pad=10)
ax.set_xlabel('トルク $T$ [任意単位]', fontsize=10)
ax.set_ylabel('回転速度 $N$ [任意単位]', fontsize=10)
ax.set_xlim(0, 35)
ax.set_ylim(40, 135)
ax.grid(True)
...
ax.text(1.0, 45, '理論式に基づく概念図（Excelに直流電動機の実測値なし）',
